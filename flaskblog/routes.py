...
def request_reset():
    ...
    if form.validate_on_submit():
        ...
        flash(message='An email has been sent with instructions to reset your password.'\
              , category='info')
        return redirect(url_for('login'))
    return render_template('request_reset.html', title='Reset Password', form=form)

@app.route('/request_reset/<token>', methods=['GET', 'POST'])
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(message="Your password has been updated. You are ready to log in not.", category='success')
        return redirect(url_for('login'))
